...
func TestReadGNUSparsePAXHeaders(t *testing.T) {
	...
	for i, v := range vectors {
		...
		tr := Reader{curr: &regFileReader{r, int64(r.Len()), 0, 0}}
...
func TestFileReader(t *testing.T) {
	...
	for i, v := range vectors {
		...
		case makeReg:
			...
			fr = &regFileReader{r, maker.size, 0, 0}
		case makeSparse:
			...
			fr = &regFileReader{r, maker.makeReg.size, 0, 0}
...
func TestReadAt(t *testing.T) {
	fi, err := os.Open("testdata/star.tar")
	if err != nil {
		t.Fatal(err)
	}
	...
	for {
		...
		fmt.Println("CONTENT222", string(out))
	}
}
